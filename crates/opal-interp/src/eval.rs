...
#[derive(Clone)]
pub(crate) struct StoredActorDef {
    #[allow(dead_code)]
    pub(crate) name: String,
    pub(crate) needs: Vec<(String, Option<String>, Option<Expr>)>,
    pub(crate) init: Option<Vec<Stmt>>,
    pub(crate) receive_cases: Vec<MatchCase>,
}

/// A stored actor instance
#[derive(Clone)]
pub(crate) struct StoredActorInstance {
...
pub struct Interpreter<W: Write> {
    ...
    pub(crate) actor_defs: Vec<StoredActorDef>,
    pub(crate) actors: Vec<StoredActorInstance>,
    ...
    pub(crate) current_actor: Option<ActorId>,
...
impl<W: Write> Interpreter<W> {
    ...
    fn call_method(
        ...
        match (&obj, method) {
            // List methods — delegated to list_methods.rs
            (Value::List(items), _) => self.call_list_method(items, method, args),
            // String methods — delegated to string_methods.rs
            (Value::String(s), _) => self.call_string_method(s, method, args),

            // Dict methods — delegated to dict_methods.rs
            ...
            // Actor methods — delegated to class_methods.rs
            (Value::ActorClass(_), _) | (Value::Actor(_), _) => {
                self.call_actor_method(&obj, method, args, &named_args)
            }

            // Class methods — delegated to class_methods.rs
            (Value::Class(class_id), _) => {
                self.call_class_method(class_id, method, args, &named_args)
            }
    ...
    fn eval_pipe(&mut self, left: &Expr, right: &Expr) -> Result<Value, EvalError> {
        ...
        match &right.kind {
            ExprKind::Identifier(name) => {
                ...
                // Try builtins
                if let Some(result) =
                    opal_stdlib::call_builtin(name, std::slice::from_ref(&arg), &mut self.writer)
                {
                    return match result {
                        Ok(opal_stdlib::BuiltinResult::Value(v)) => Ok(v),
                        Ok(opal_stdlib::BuiltinResult::Void) => Ok(Value::Null),
    ...
    pub(crate) fn class_implements_protocol(&self, class_id: ClassId, protocol_name: &str) -> bool {
    ...
    fn apply_format_spec(formatted: &str, val: &Value, spec: &str) -> String {
        ...
        if let Some(rest) = spec.strip_prefix('.') {
            let num_str = rest.trim_end_matches('f');
